...
function register_user($conn,$param){
    ...
    $sfctoken ;
    $sql = "INSERT INTO user(name,emailId,phone,password,userType,otp,sfctoken)
    VALUES ('".$param->name."','".$param->email."','".$param->phone."','".$param->password."',".$param->usertype.",'".$otp."','".$sfctoken."')";
    $isInserted = FALSE;
...
function login_user($conn,$param){
    ...
    $sql1 = " UPDATE user SET sfctoken = '".$sfctoken."' WHERE emailId = '".$email."'"; 
...
function classdetails($conn,$param)
{
    ...
    $sql1 = "SELECT liveId FROM liveclass WHERE cId ='".$row["cId"]."' AND liveId !='".$rowr["liveId"]."'";
        $result1 = $conn->query($sql1);
       if($result1->num_rows>0){
            while(  $row1 = $result1->fetch_assoc()){
...
function own_class1($conn,$param){
    ...
    $lid=$param->lid;
    $res = [];
    $sql2 = "SELECT liveclass.liveId,liveclass.cId,liveclass.title, liveclass.descr,timetable.day,timetable.from_time,timetable.to_time FROM liveclass JOIN timetable ON liveclass.liveId = timetable.liveId  WHERE liveclass.tId ='".$id."' AND liveclass.liveId ='".$lid."' ";
    $result2 = $conn->query($sql2);
    if($result2->num_rows>0){
        while(  $row2 = $result2->fetch_assoc()){
            array_push($res,$row2);
...
function add_live_class1($conn,$param)
{
    ...
    $sql = "UPDATE liveclass SET title = '".$param->title."',descr = '".$param->descr."' WHERE tId = '".$tId."' AND liveId ='".$lid."'";
